...
public class KeyBoardPlayer : MonoBehaviour
{
  [SerializeField, Tooltip("walkingSpeed")]
    ...
    void Update()
    {
        ...
        bool isRunning = Input.GetButton("kRun");
        float curSpeedX = canMove ? (isRunning ? runningSpeed : walkingSpeed) * Input.GetAxis("kVertical") : 0;
        float curSpeedY = canMove ? (isRunning ? runningSpeed : walkingSpeed) * Input.GetAxis("kHorizontal") : 0;
        float movementDirectionY = moveDirection.y;
        moveDirection = (forward * curSpeedX) + (right * curSpeedY);

        if (Input.GetButton("kJump") && canMove && characterController.isGrounded)
        {
        ...
        if (canMove)
        {
            rotationX += -Input.GetAxis("kMouse Y") * lookSpeed;
            rotationX = Mathf.Clamp(rotationX, -lookXLimit, lookXLimit);
            playerCamera.transform.localRotation = Quaternion.Euler(rotationX, 0, 0);
            transform.rotation *= Quaternion.Euler(0, Input.GetAxis("kMouse X") * lookSpeed, 0);
        }
